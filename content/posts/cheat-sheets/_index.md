---
title: "Louis Neilson's Cheat Sheets"
url: "/cheat-sheets/"
menu:
  sidebar:
    name: Cheat Sheets
    identifier: category
    weight: 20
---

# Louis Neilson's Cheat Sheets
*Cheat sheets I make for myself and hope others might find useful*

Sheets currently available:
1. [SQL Operators](./sql-operators/)
These are tables of SQL commands that can be used to modify the logic of SQL queries. The content largely comes from [W3School's SQL Operators](https://www.w3schools.com/sql/sql_operators.asp) page, but I have provided some additional context and clarification for my own needs. 

# Licence

This work is licensed under the Creative Commons Attribution–NonCommercial 4.0 International Licence (CC BY-NC 4.0).
You may copy, adapt, and share the material for any non-commercial purpose, provided that appropriate credit is given.

Full licence text: [https://creativecommons.org/licenses/by-nc/4.0/](https://creativecommons.org/licenses/by-nc/4.0/)

## Commercial use:
For-profit institutions must obtain explicit written permission before using or adapting this material.
Enquiries: [hello@louisneilson.com](mailto:hello@louisneilson.com)